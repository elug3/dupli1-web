...
2. **Cart read** (`GET /api/v1/cart`) keeps stale lines but **silently drops enrichment** when `resolveVariant` fails (`cart/pkg/service/service.go` → `enrichCart`). The client sees `sku` / `sku_id` + `quantity` but empty `product_id`, `unit_price_won`, etc. There is no explicit `unavailable` flag or `missing` list.
...
**Pricing rule:** `subtotal_won` must **exclude** unavailable lines (today enrichment failure already yields `unit_price_won: 0`; make that explicit and documented).

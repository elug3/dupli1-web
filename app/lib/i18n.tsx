...
const dictionaries: Record<LanguageCode, Record<string, string>> = {
  en: {
    ...
    "checkout.validPccc": "Enter a valid PCCC (P followed by 12 digits)",
    ...
    "checkout.city": "District",
    "checkout.province": "Province / City",
    "checkout.selectProvince": "Select province or city",
    "checkout.selectDistrict": "Select district",
    "checkout.selectProvinceFirst": "Select a province first",
    "checkout.zip": "ZIP Code",
    "checkout.pccc": "Personal Customs Clearance Code",
    "checkout.pcccPlaceholder": "P123456789012",
    "checkout.pcccHint":
      "Required by Korean customs to clear your shipment (format: P followed by 12 digits).",
    ...
    "checkout.methodCreditCardHint":
      "You are redirected to the card payment gateway to finish paying",
    "checkout.methodSecureRedirect": "Secure redirect",
    ...
    "checkout.paymentNote":
      "Your card is charged by the payment gateway after you place the order. Stock is reserved as soon as the order is created.",
    "checkout.stepShipping": "Shipping Address",
    "checkout.stepPayment": "Payment Method",
    "checkout.stepReview": "Check & Complete",
    ...
    "checkout.reviewNote": "Check everything below, then place your order.",
    "checkout.reviewItems": "Items ({count})",
    "checkout.reviewQuantity": "Qty {count}",
    "checkout.edit": "Edit",
    "checkout.editSection": "Edit {section}",
    ...
    "profile.ordersLoadFailed": "Couldn't load orders. Please try again.",
    "profile.statusPending": "Pending",
    "profile.statusPaid": "Paid",
    "profile.statusConfirmed": "Preparing",
    "profile.statusInTransit": "Shipped",
    "profile.statusDelivered": "Delivered",
    "profile.statusFulfilled": "Completed",
    "profile.statusDisputed": "Under review",
    "profile.statusCanceled": "Canceled",
    "profile.orderItems": "{count} item",
    "profile.orderItemsPlural": "{count} items",
    ...
    "profile.faqCancelOrderAnswer":
      "You can cancel for an immediate refund until a manager confirms the order (confirmation is required within 2 hours of payment). After confirmation, through shipping and delivery, cancellation goes to a manager and is processed after they confirm — they have 2 hours, after which the refund is approved automatically.",
    ...
    "profile.deliveredPrompt": "Did you receive this order?",
    "profile.confirmReceipt": "Yes, I received it",
    "profile.reportNotReceived": "I didn't receive it",
    "profile.confirmReportNotReceived":
      "Report that this order was never received? A manager will review it.",
    "profile.responding": "Sending…",
    "profile.respondFailed": "Could not send your response. Please try again.",
    "profile.disputeUnderReview": "We've flagged this order for review — we'll follow up soon.",
  ...
  ko: {
    ...
    "checkout.validPccc": "유효한 개인통관고유부호를 입력하세요 (P + 12자리 숫자)",
    ...
    "checkout.selectProvince": "시/도 선택",
    "checkout.selectDistrict": "시/군/구 선택",
    "checkout.selectProvinceFirst": "시/도를 먼저 선택하세요",
    "checkout.zip": "우편번호",
    "checkout.pccc": "개인통관고유부호",
    "checkout.pcccPlaceholder": "P123456789012",
    "checkout.pcccHint":
      "통관을 위해 필요합니다. (형식: P + 숫자 12자리)",
    "checkout.country": "국가",
    ...
    "checkout.methodCreditCardHint":
      "결제 게이트웨이로 이동하여 카드 결제를 완료합니다",
    "checkout.methodSecureRedirect": "안전 결제 이동",
    ...
    "checkout.paymentNote":
      "주문하기를 누르면 결제 게이트웨이에서 카드 결제가 진행됩니다. 주문이 생성되는 즉시 재고가 확보됩니다.",
    "checkout.stepShipping": "배송지",
    "checkout.stepPayment": "결제 수단",
    "checkout.stepReview": "확인 및 완료",
    "checkout.stepCount": "총 {total}단계 중 {current}단계",
    "checkout.continueToPayment": "결제 수단 선택",
    "checkout.continueToReview": "주문 확인",
    "checkout.reviewNote": "아래 내용을 확인한 뒤 주문을 완료하세요.",
    "checkout.reviewItems": "상품 ({count})",
    "checkout.reviewQuantity": "수량 {count}",
    "checkout.edit": "수정",
    "checkout.editSection": "{section} 수정",
    ...
    "profile.ordersLoadFailed": "주문을 불러올 수 없습니다. 다시 시도해주세요.",
    "profile.statusPending": "결제 대기",
    "profile.statusPaid": "결제 완료",
    "profile.statusConfirmed": "상품 준비 중",
    "profile.statusInTransit": "배송 중",
    "profile.statusDelivered": "배송 완료",
    "profile.statusFulfilled": "구매 확정",
    "profile.statusDisputed": "확인 중",
    "profile.statusCanceled": "취소됨",
    "profile.orderItems": "{count}개 상품",
    "profile.orderItemsPlural": "{count}개 상품",
    "profile.profile": "프로필",
    "profile.name": "이름",
    ...
    "profile.faqCancelOrderAnswer":
      "매니저가 주문을 확인하기 전(결제 후 2시간 이내 확인)에는 즉시 환불됩니다. 주문 확인 이후부터 배송 완료까지의 취소는 매니저 확인 후 처리되며, 매니저는 2시간 이내에 응답해야 합니다. 기한이 지나면 환불이 자동 승인됩니다.",
    ...
    "profile.deliveredPrompt": "주문하신 상품을 받으셨나요?",
    "profile.confirmReceipt": "네, 받았어요",
    "profile.reportNotReceived": "받지 못했어요",
    "profile.confirmReportNotReceived": "이 주문을 받지 못했다고 신고할까요? 매니저가 확인합니다.",
    "profile.responding": "전송 중…",
    "profile.respondFailed": "응답을 보내지 못했습니다. 다시 시도해 주세요.",
    "profile.disputeUnderReview": "이 주문을 확인 중입니다 — 곧 안내드리겠습니다.",
  ...
  zh: {
    ...
    "checkout.validPccc": "请输入有效的个人通关识别码（P + 12位数字）",
    ...
    "checkout.selectProvince": "选择省市",
    "checkout.selectDistrict": "选择区/市/县",
    "checkout.selectProvinceFirst": "请先选择省市",
    "checkout.zip": "邮编",
    "checkout.pccc": "个人通关识别码",
    "checkout.pcccPlaceholder": "P123456789012",
    "checkout.pcccHint": "韩国海关清关必填项（格式：P + 12位数字）。",
    "checkout.country": "国家",
    ...
    "checkout.methodCreditCardHint": "跳转至银行卡支付网关完成付款",
    "checkout.methodSecureRedirect": "安全跳转",
    ...
    "checkout.paymentNote":
      "提交订单后由支付网关完成扣款。订单创建后立即锁定库存。",
    "checkout.stepShipping": "配送地址",
    "checkout.stepPayment": "付款方式",
    "checkout.stepReview": "确认并完成",
    "checkout.stepCount": "第 {current} 步，共 {total} 步",
    "checkout.continueToPayment": "继续选择付款方式",
    "checkout.continueToReview": "继续确认订单",
    "checkout.reviewNote": "请核对以下信息后提交订单。",
    "checkout.reviewItems": "商品 ({count})",
    "checkout.reviewQuantity": "数量 {count}",
    "checkout.edit": "修改",
    "checkout.editSection": "修改{section}",
    ...
    "profile.ordersLoadFailed": "无法加载订单，请重试。",
    "profile.statusPending": "待付款",
    "profile.statusPaid": "已付款",
    "profile.statusConfirmed": "备货中",
    "profile.statusInTransit": "配送中",
    "profile.statusDelivered": "已送达",
    "profile.statusFulfilled": "已完成",
    "profile.statusDisputed": "审核中",
    "profile.statusCanceled": "已取消",
    "profile.orderItems": "{count} 件",
    "profile.orderItemsPlural": "{count} 件",
    ...
    "profile.faqCancelOrderAnswer":
      "经理确认订单前（付款后需在 2 小时内确认）可立即退款。确认之后直到送达前的取消需经理确认后处理，经理须在 2 小时内回应；超时将自动批准退款。",
    ...
    "profile.deliveredPrompt": "您收到这个订单了吗？",
    "profile.confirmReceipt": "是的，已收到",
    "profile.reportNotReceived": "没有收到",
    "profile.confirmReportNotReceived": "举报未收到此订单？经理将进行审核。",
    "profile.responding": "发送中…",
    "profile.respondFailed": "无法发送您的回复，请重试。",
    "profile.disputeUnderReview": "我们正在审核此订单，很快会与您联系。",

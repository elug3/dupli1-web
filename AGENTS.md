...
**Promotional codes, not coupons.** One term on every surface: **프로모션 코드 / "promotional code."** Renamed from `coupon` on 2026-09-16 (backend [docs/product-promotion-rename.md](https://github.com/elug3/dupli1/blob/master/docs/product-promotion-rename.md)): `redeemPromotion` / `applySessionPromotion`, the `/api/promotions/redeem` and `…/sessions/:id/promotion` routes, and `promotionCode` on mapped orders and sessions. The wallet section uses `profile.promotionCodes` — `profile.promotions` is the marketing opt-in and means something else. `mapOrder` / `mapSession` read `promotion_code` and fall back to `coupon_code`, because order emits both for one release; drop the fallback when that window closes. The profile wallet reads real entitlements from `POST /api/v1/products/promotions/me` (`app/lib/promotion-wallet.ts`), each judged against the current cart so an unusable code says why rather than sitting there looking available. Lapsed and revoked entitlements drop to the bottom instead of vanishing. The customer id comes from the session token, never the request.

...
**Currency is KRW only.** Display and enter all product prices in Korean Won — do not convert to USD or other currencies by language. Cart/order/payment JSON fields named `*_krw` are **whole KRW won** (zero-decimal minor units) — never divide by 100 for display.
